...
public interface DictMapper extends BaseMapper<Dict> {
    ...
    @Select("SELECT * FROM kb_dict WHERE dict_code = #{dictCode} AND deleted = 0")
    Dict selectByDictCode(@Param("dictCode") String dictCode);

    @Select("SELECT * FROM kb_dict WHERE dict_type = #{dictType} AND status = 1 AND deleted = 0 ORDER BY sort")
